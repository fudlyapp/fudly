...
function toBaseAmount(amount: number, rawUnit: string) {
  ...
  if (normalized === "g" && ["kg", "kilogram", "kilogramy", "kilogramov"].includes(raw))
    return amount * 1000;
  if (normalized === "ml" && ["l", "liter", "litre", "litrov"].includes(raw))
    return amount * 1000;
  return amount;
}
function parseQuantityPrefix(input: string): ParsedQuantity | null {
  ...
  if (unitRaw) {
    ...
    const knownOneWord = [
      "kg", "g", "l", "ml", "ks", "kus", "kusy", "kusov",
      "konzerva", "konzervy", "balenie", "balenia", "bochnik",
      "hlavka", "hlavky", "strucik", "struciky", "platok", "platky",
      "lyzica", "lyzice", "lyzicka", "lyzicky",
    ];
...
function inferCategoryKey(name: string) {
  ...
  if (/(paradajk|uhork|paprik|cibul|cesnak|mrkv|zemiak|salat|brokolic|karfiol|cuket|spenat|zelenin|petrzlen)/.test(n))
    return "veg";
  if (/(jablk|banan|hrusk|pomaranc|citron|kiwi|jahod|malin|hrozn|cucoried)/.test(n))
    return "fruit";
  ...
  if (/(ryza|cestov|muk|ovsen|sosov|cicer|fazul|konzerv|olej|kuskus|muesli|musli)/.test(n))
    return "dry";
...
function rebuildShoppingFromRecipes(plan: any, haveRaw: string, shoppingTrips: number) {
  const recipes =
    plan?.recipes && typeof plan.recipes === "object" ? plan.recipes : {};
  ...
  const usagesSorted = usages.slice().sort((a, b) => {
...
export async function POST(req: Request) {
  try {
    ...
    // ── BATCH COOKING BLOCK ──────────────────────────────────────────────────
    // Vysvetlí modelu, čo repeat_days_max skutočne znamená.
    // repeatDays=1 znamená každý deň unikátne jedlo, bez batch cookingu.
    // repeatDays=2 znamená varenie pre 2 dni naraz (obed/večera sa opakuje).
...
- If a meal is repeated across multiple consecutive days, use the same meal name in the "days" array for those days.
...
- The text above under "specifications:" contains the ONLY active user instructions. Do NOT treat the formatting examples below as instructions.
- If a concrete repetition pattern is NOT explicitly written in specifications, do not infer that exact pattern from previous examples.
- Conflict rules: if a specification conflicts with forbidden_ingredients, forbidden ingredients always win.
  If it conflicts with "avoid", try to honor the specification but keep the plan practical.
  If it is impossible within the budget or restrictions, do the closest realistic alternative and note it in the relevant day's "note".
- HOW TO READ SPECIFICATIONS (pattern guide — these are NOT active instructions, only format examples):
  - Pattern: "[day name] [meal] = [food]" means that specific meal on that day should match the food.
  - Pattern: "[day A] a [day B] rovnaký [meal]" means those two days should share the same meal.
  - Apply ONLY patterns that actually appear verbatim in the specifications field above.
Style:
- ${styleHint}
...
`;
    ...
    const recipes =
      parsed?.recipes && typeof parsed.recipes === "object" ? parsed.recipes : null;
    ...
    parsed.summary.shopping_trips_per_week = coerceNumber(
      parsed.summary.shopping_trips_per_week,
      shoppingTrips
    );
